...
class SetpC_USA_DX extends React.Component {
    constructor(props) {
        super(props);
    ...
    handleSubmit(e){
        ...
        var jsonEntity={
            ...
            AcademicType:e.target["AcademicType"].value,
            AcademicScore:e.target["AcademicScore"].value,
            IsTest:this.state.IsTestItem.join(";")
        };
        var entity = {
        ...
        if(jsonEntity.Major=="选就读专业类型"||jsonEntity.Major==""||jsonEntity.Major==null){
        ...
        if(jsonEntity.LanguageType=="选英语考试类型"||jsonEntity.LanguageType==""||jsonEntity.LanguageType==null){
            Common.alert("请选择英语考试类型！");
            return;
        }
        ...
        if(jsonEntity.AcademicType=="标准化入学考试"||jsonEntity.AcademicType==""||jsonEntity.AcademicType==null){
            Common.alert("请选择标准化入学考试类型！");
            return;
        }
        if(jsonEntity.AcademicScore=="请选择"||jsonEntity.AcademicScore==""||jsonEntity.AcademicScore==null){
            Common.alert("请输入学术考试分数！");
            return;
        }
        if(this.state.IsTestItem.length<=0){
            Common.alert("请选择是否考过！");
            return;
        }
        ...
        if(Common.getCookie("userLoginSuccess")!=null&&Common.getCookie("userLoginSuccess").length > 0){
            var flagDouble;
            this.props.SelectItem.map(function(obj){
                (obj=="本科双录取"||"硕士双录取"==obj)?flagDouble=true:"";
            });
            if(flagDouble&&this.props.SelectItem.length==1){
                window.location.href="/USAProjectPage";
            }else {
    ...
    isTestHandleChange(e){
        var flag = true, oSelectItem = new Array();
        var e1=e.currentTarget.value;
        if(this.state.IsTestItem.length==0){
            oSelectItem.push(e1);
            this.setState({IsTestItem:oSelectItem});
        }else{
            oSelectItem = this.state.IsTestItem;
            ...
            if(flag){
              ...
              for (var i = 0; i < oSelectItem.length ; i++) {
                  if(oSelectItem[i] != e1)
                  {
                      newoSelectItem.push(oSelectItem[i]);
                  }
              }
              this.setState({IsTestItem:newoSelectItem});
            }
        }
        //console.log(this.state.IsTestItem);
    }
    render() {
        return (
            <div>
                        ...
                        <div className="ws-education1">
                            <div className="education-box">
                                <div className="content">
                                    ...
                                    <dl>
                                        <dd>
                                            <Select value={this.state.Major==null?"选就读专业类型":this.state.Major} style={{width:160,height:34}} optionFilterProp="desc"  renderDropdownToBody={true} onChange={this.handleChangeC.bind(this)}>
                                                <Option value="选就读专业类型" desc="选就读专业类型">选就读专业类型</Option>
                                        ...
                                        <dd>
                                            <input type="text" id="MajorName" placeholder="就读专业名称" />
                                        </dd>
                                    </dl>
                                    <dl>
                                        <dd>
                                            ...
                                            <input type="hide" id="AvgScoreType" name="AverageScoresType" className="inputHide" value={this.state.AvgScoreType}/>
                                        </dd>
                                        <dd>
                                            <input type="text" id="AvgScore" name="AverageScores" placeholder="平均成绩" />
                                        </dd>
                                    </dl>
                                    <dl>
                                        <dd>
                                            <Select value={this.state.LanguageType==null?"选英语考试类型":this.state.LanguageType} style={{width:160,height:34}} optionFilterProp="desc"  renderDropdownToBody={true} onChange={this.handleChangeE.bind(this)}>
                                                <Option value="选英语考试类型" desc="选英语考试类型">选择英语考试类型</Option>
                                    ...
                                    <dl>
                                        <dd>
                                            <Select value={this.state.HopeMajor==null?"选希望就读专业":this.state.HopeMajor} style={{width:160,height:34}} optionFilterProp="desc"  renderDropdownToBody={true} onChange={this.handleChangeF.bind(this)}>
                                                <Option value="选希望就读专业" desc="选希望就读专业">选择希望就读专业</Option>
                                                <Option value="工科" desc="工科">工科</Option>
                                                <Option value="经济金融" desc="经济金融">经济金融</Option>
                                                <Option value="商科" desc="商科">商科</Option>
                                                <Option value="社会科学" desc="社会科学">社会科学</Option>
                                    ...
                                    <dl>
                                        <dd>
                                            <Select value={this.state.AcademicType==null?"标准化入学考试":this.state.AcademicType} style={{width:160,height:34}} optionFilterProp="desc"  renderDropdownToBody={true} onChange={this.handleChangeH.bind(this)}>
                                                <Option value="标准化入学考试" desc="标准化入学考试">标准化入学考试</Option>
                                                <Option value="GRE" desc="GRE">GRE</Option>
                                                <Option value="GMAT" desc="GMAT">GMAT</Option>
                                                <Option value="SAT" desc="SAT">SAT</Option>
                                                <Option value="ACT" desc="ACT">ACT</Option>
                                            </Select>
                                            <input type="hide" id="AcademicType" className="inputHide" value={this.state.AcademicType}/>
                                        </dd>
                                        <dd>
                                            <input type="text" id="AcademicScore" placeholder="学术考试成绩" />
                                        </dd>
                                    </dl>
                                    <dl>
                                        <dd>
                                        ...
                                        <dd>
                                            <Checkbox onChange={this.isTestHandleChange.bind(this)} name="USA_Success" defaultValue="ESL"/>
                                            <label for="chk_IsTest">ESL</label>&nbsp;&nbsp;
                                            <Checkbox onChange={this.isTestHandleChange.bind(this)} name="USA_Success" defaultValue="SAT2"/>
                                            <label for="chk_IsTest">SAT2</label>&nbsp;&nbsp;
                                            <Checkbox onChange={this.isTestHandleChange.bind(this)} name="USA_Success" defaultValue="IB"/>
                                            <label for="chk_IsTest">Ib</label>&nbsp;&nbsp;
                                            <Checkbox onChange={this.isTestHandleChange.bind(this)} name="USA_Success" defaultValue="AP"/>
                                            <label for="chk_IsTest">AP</label>&nbsp;&nbsp;
                                            <Checkbox onChange={this.isTestHandleChange.bind(this)} name="USA_Success" defaultValue="A-Level"/>
                                            <label for="chk_IsTest">A-Level</label>&nbsp;&nbsp;
                                        </dd>
                                    </dl>
                                </div>
